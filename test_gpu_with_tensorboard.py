...
from models import make_image_embeddings_cnn_with_tensorboard
import numpy as np
...
with tf.name_scope('CNN_Network'):
    ...
    w1_hist = tf.summary.histogram('conv1_weights', w1)
    w2_hist = tf.summary.histogram('conv2_weights', w2)
    w3_hist = tf.summary.histogram('conv3_weights', w3)
    w4_hist = tf.summary.histogram('full_connected_weights', w4)
    p_keep_conv_scalar = tf.summary.scalar('conv\'s drop out rate', p_keep_conv)
    p_keep_hidden_scalar = tf.summary.scalar('conv\'s drop out rate', p_keep_conv)

    image_embedding = make_image_embeddings_cnn_with_tensorboard.model(X, w1, w2, w3, w4, p_keep_conv, p_keep_hidden)
    image_embedding_hist = tf.summary.histogram('image_embedding', image_embedding)
...
with tf.name_scope('output_layer'):
    ...
    w_o_hist = tf.summary.histogram('Output_layer\'s weights', w_o)
    bias_o_hist = tf.summary.histogram('Output_layer\'s bias', bias)
    logits_hist = tf.summary.histogram('Output_layer', logits)

with tf.name_scope('softmax_activation'):
    softmaxed_logits = tf.nn.softmax(logits=logits)

    softmax_hist = tf.summary.histogram('logits after activation', softmaxed_logits)
...
with tf.Session() as sess:
    ...
    writer = tf.summary.FileWriter('./logs/image_tagging_network')
    ...
    for start in range(0, len(unique_images), batch_size):
        ...
        summary = sess.run(merged_summary, feed_dict={X:x_data_list})
